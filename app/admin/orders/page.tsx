...
const statusColors: Record<string, { bg: string; text: string }> = {
  ...
  processing: { bg: "rgba(14,165,233,0.12)",  text: "#0284c7" },
  shipped:    { bg: "rgba(139,92,246,0.12)",  text: "#7c3aed" },
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 32, fontWeight: 700, color: "#1a1a2e", marginBottom: 4 }}>Orders</h1>
      ...
      {loading ? (
        ...
        <div className="glass-card" style={{ padding: "72px 24px", borderRadius: 22, textAlign: "center" }}>
          ...
          <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: "#1a1a2e", marginBottom: 8 }}>No Orders Yet</h3>
          <p style={{ color: "#9ca3af" }}>Orders will appear here once customers start purchasing.</p>
        </div>
      ) : (
                ...
                {orders.map((order, i) => (
                  <tr key={order._id} style={{ borderTop: "1px solid rgba(139,92,246,0.06)", background: i % 2 === 0 ? "transparent" : "rgba(245,243,255,0.3)" }}>
                    ...
                    <td style={{ padding: "14px 20px" }}>
                      ...
                      <p style={{ fontSize: 11, color: "#9ca3af", margin: "2px 0 0" }}>{order.user?.email}</p>
                    </td>
                    <td style={{ padding: "14px 20px", fontSize: 13, color: "#374151" }}>
                      {order.items?.length || 0} item{order.items?.length !== 1 ? "s" : ""}
                    </td>
                    <td style={{ padding: "14px 20px", fontSize: 15, fontWeight: 700, color: "#7c3aed" }}>
                    ...
                    <td style={{ padding: "14px 20px" }}>
                      <div style={{ position: "relative", display: "inline-block" }}>
                        ...
                        <ChevronDown size={12} style={{ position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: statusColors[order.status]?.text || "#6b7280" }} />
                      </div>
                    </td>
