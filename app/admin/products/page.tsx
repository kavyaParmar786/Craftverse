...
function ProductModal({ product, onClose, onSave, token }: {
  ...
  const [form, setForm] = useState<FormState>(
    product
      ? { name: product.name, price: String(product.price), category: product.category, description: product.description, stock: String(product.stock), featured: product.featured, image: product.image }
      : { ...emptyForm }
  );
  ...
  return (
    <div style={{ position:"fixed",inset:0,zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(0,0,0,0.25)",backdropFilter:"blur(4px)",padding:24 }}>
      <div className="glass-card" style={{ width:"100%",maxWidth:540,borderRadius:24,padding:"36px",maxHeight:"90vh",overflowY:"auto" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:28 }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif",fontSize:22,fontWeight:700,color:"#1a1a2e",margin:0 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display:"grid",gap:16 }}>
          <div>
            <label style={{ display:"block",fontSize:13,fontWeight:600,color:"#374151",marginBottom:6 }}>Product Name *</label>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:14 }}>
            <div>
              <label style={{ display:"block",fontSize:13,fontWeight:600,color:"#374151",marginBottom:6 }}>Price (₹) *</label>
            ...
            <div>
              <label style={{ display:"block",fontSize:13,fontWeight:600,color:"#374151",marginBottom:6 }}>Stock</label>
          ...
          <div>
            ...
            <input className="craft-input" value={form.image} onChange={e=>set("image",e.target.value)} placeholder="https://..."/>
          </div>
          <div>
            ...
            <textarea className="craft-input" required rows={3} style={{ resize:"vertical" }}
              value={form.description} onChange={e=>set("description",e.target.value)} placeholder="Describe the product..."/>
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            <input type="checkbox" id="featured" checked={form.featured}
              onChange={e=>set("featured",e.target.checked)}
              style={{ width:16,height:16,accentColor:"#8b5cf6" }}/>
            <label htmlFor="featured" style={{ fontSize:14,color:"#374151",cursor:"pointer" }}>Mark as Featured</label>
          </div>
...
export default function AdminProductsPage() {
  ...
  const catColor:Record<string,string>={charts:"#8b5cf6",models:"#0ea5e9","3d-printing":"#10b981",clothes:"#fb7185"};

  return (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:32 }}>
        <div>
          <h1 style={{ fontFamily:"'Playfair Display',serif",fontSize:32,fontWeight:700,color:"#1a1a2e",marginBottom:4 }}>Products</h1>
          <p style={{ color:"#6b7280",fontSize:14 }}>{products.length} products in catalog</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-card" style={{ padding:"72px 24px",borderRadius:22,textAlign:"center" }}>
          ...
          <h3 style={{ fontFamily:"'Playfair Display',serif",fontSize:20,color:"#1a1a2e",marginBottom:8 }}>No Products Yet</h3>
          <p style={{ color:"#9ca3af",marginBottom:20 }}>Add your first product to get started.</p>
              ...
              <thead>
                <tr style={{ background:"rgba(139,92,246,0.04)" }}>
                  {["Product","Category","Price","Stock","Featured","Actions"].map(h=>(
                    <th key={h} style={{ padding:"14px 20px",textAlign:"left",fontSize:12,fontWeight:600,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em",whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {products.map((p,i)=>(
                  <tr key={p._id} style={{ borderTop:"1px solid rgba(139,92,246,0.06)",background:i%2===0?"transparent":"rgba(245,243,255,0.3)" }}>
                    <td style={{ padding:"14px 20px" }}>
                      <p style={{ fontSize:14,fontWeight:600,color:"#1a1a2e",margin:0 }}>{p.name}</p>
                      <p style={{ fontSize:12,color:"#9ca3af",margin:"2px 0 0",maxWidth:240,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{p.description}</p>
                    </td>
                    <td style={{ padding:"14px 20px" }}>
                    ...
                    <td style={{ padding:"14px 20px",fontSize:15,fontWeight:700,color:"#7c3aed" }}>₹{p.price.toLocaleString("en-IN")}</td>
                    <td style={{ padding:"14px 20px",fontSize:14,color:p.stock<5?"#ef4444":"#374151",fontWeight:p.stock<5?700:400 }}>{p.stock}</td>
                    <td style={{ padding:"14px 20px" }}>
                      {p.featured?<span style={{ padding:"3px 10px",borderRadius:6,background:"rgba(139,92,246,0.1)",color:"#7c3aed",fontSize:12,fontWeight:600 }}>★ Yes</span>:<span style={{ color:"#d1d5db",fontSize:13 }}>—</span>}
                    </td>
                    <td style={{ padding:"14px 20px" }}>
                      <div style={{ display:"flex",gap:8 }}>
                        <button onClick={()=>setModal({open:true,product:p})} style={{ padding:"6px 10px",borderRadius:8,border:"none",background:"rgba(139,92,246,0.08)",cursor:"pointer",color:"#7c3aed" }}><Pencil size={14}/></button>
