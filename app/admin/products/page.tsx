              ...
              <thead>
                <tr style={{ background:"rgba(139,92,246,0.04)" }}>
                  {["Image","Product","Category","Price","Stock","Featured","Actions"].map(h=>(
                    <th key={h} style={{ padding:"14px 16px",textAlign:"left",fontSize:12,fontWeight:600,color:"#6b7280",textTransform:"uppercase",letterSpacing:"0.05em",whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {products.map((p,i)=>(
                  <tr key={p._id} style={{ borderTop:"1px solid rgba(139,92,246,0.06)",background:i%2===0?"transparent":"rgba(245,243,255,0.3)" }}>
                    <td style={{ padding:"12px 16px" }}>
                      <div style={{ width:44,height:44,borderRadius:10,overflow:"hidden",background:"linear-gradient(135deg,#f5f3ff,#e0f2fe)",flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20 }}>
                        {p.image
                    ...
                    <td style={{ padding:"12px 16px" }}>
                      ...
                      <p style={{ fontSize:12,color:"#9ca3af",margin:"2px 0 0",maxWidth:200,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{p.description}</p>
                    </td>
                    <td style={{ padding:"12px 16px" }}>
                    ...
                    <td style={{ padding:"12px 16px",fontSize:15,fontWeight:700,color:"#7c3aed" }}>₹{p.price.toLocaleString("en-IN")}</td>
                    <td style={{ padding:"12px 16px",fontSize:14,color:p.stock<5?"#ef4444":"#374151",fontWeight:p.stock<5?700:400 }}>{p.stock}</td>
                    <td style={{ padding:"12px 16px" }}>
                      {p.featured?<span style={{ padding:"3px 10px",borderRadius:6,background:"rgba(139,92,246,0.1)",color:"#7c3aed",fontSize:12,fontWeight:600 }}>★ Yes</span>:<span style={{ color:"#d1d5db" }}>—</span>}
                    </td>
                    <td style={{ padding:"12px 16px" }}>
