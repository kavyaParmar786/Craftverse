    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.25)", backdropFilter: "blur(4px)", padding: 24 }}>
      <div className="glass-card" style={{ width: "100%", maxWidth: 600, borderRadius: 24, padding: "36px", maxHeight: "90vh", overflowY: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 700, color: "#2C1A0E", margin: 0 }}>New Post</h2>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: 16 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Title *</label>
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Excerpt *</label>
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Content *</label>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Author</label>
              <input className="craft-input" value={form.author} onChange={e => setForm((f: any) => ({ ...f, author: e.target.value }))} />
            </div>
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Tags (comma-separated)</label>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
        <div>
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 32, fontWeight: 700, color: "#2C1A0E", marginBottom: 4 }}>News & Blog</h1>
          <p style={{ color: "#8B6F5E", fontSize: 14 }}>{posts.length} published posts</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="glass-card" style={{ padding: "72px 24px", borderRadius: 22, textAlign: "center" }}>
          ...
          <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 20, color: "#2C1A0E", marginBottom: 8 }}>No Posts Yet</h3>
          <p style={{ color: "#B89080", marginBottom: 20 }}>Publish your first article.</p>
          ...
          {posts.map((post, i) => (
            <div key={post._id} className="glass-card" style={{ padding: "20px 24px", borderRadius: 18, display: "flex", alignItems: "center", gap: 16 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 15, fontWeight: 600, color: "#2C1A0E", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{post.title}</p>
                <p style={{ fontSize: 12, color: "#B89080", margin: "4px 0 0" }}>{post.author} · {new Date(post.createdAt).toLocaleDateString("en-IN")} · {post.tags?.join(", ")}</p>
              </div>
              <span style={{ padding: "3px 10px", borderRadius: 6, fontSize: 11, fontWeight: 600, background: "rgba(201,123,99,0.1)", color: "#A85E48" }}>Published</span>
            </div>
          ))}
        </div>
      )}
