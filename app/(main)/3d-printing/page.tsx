  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/requests", {
        ...
        body: JSON.stringify({ ...form, type: "3d-printing", budget: Number(form.budget) }),
      });
          ...
          {submitted ? (
            ...
            <form onSubmit={handleSubmit} className="glass-card" style={{ padding: "36px", borderRadius: 24 }}>
              ...
              <div style={{ display: "grid", gap: 18 }}>
                ...
                {/* File upload note */}
                <div style={{ padding: "16px", borderRadius: 12, background: "rgba(139,92,246,0.05)", border: "1px dashed rgba(139,92,246,0.25)", textAlign: "center" }}>
                  <FileBox size={24} color="#8b5cf6" style={{ marginBottom: 8 }} />
                  <p style={{ fontSize: 13, color: "#6b7280", margin: 0 }}>
                    Have an STL/OBJ file? Share it via WhatsApp or email after submitting. We'll request it from you.
                  </p>
                </div>
