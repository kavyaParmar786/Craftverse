...
function CustomOrderForm() {
  ...
  if (submitted) return (
    <div style={{ textAlign: "center", padding: "60px 24px", background: "rgba(255,241,242,0.8)", borderRadius: 24, border: "1px solid rgba(201,123,99,0.2)" }}>
      <CheckCircle2 size={56} color="#C97B63" style={{ marginBottom: 16 }} />
      <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 24, color: "#2C1A0E", marginBottom: 8 }}>Order Request Submitted!</h3>
      <p style={{ color: "#8B6F5E", marginBottom: 24 }}>We'll confirm your design and pricing within 24 hours.</p>
      <Link href="/" className="btn-primary" style={{ background: "linear-gradient(135deg, #C97B63, #A85E48)" }}>Back to Home <ArrowRight size={15} /></Link>
    </div>
  );
  ...
  return (
    <form onSubmit={handleSubmit} className="glass-card" style={{ padding: "40px", borderRadius: 24 }}>
      <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 26, fontWeight: 700, color: "#2C1A0E", marginBottom: 28 }}>
        <Shirt size={22} color="#C97B63" style={{ display: "inline", marginRight: 8 }} />
      ...
      <div style={{ display: "grid", gap: 18 }}>
        <div>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Design Title *</label>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14 }}>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Type</label>
          ...
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Size</label>
            <select className="craft-input" value={form.size} onChange={e => setForm(f => ({ ...f, size: e.target.value }))}>
              {sizes.map(s => <option key={s}>{s}</option>)}
            </select>
          </div>
          <div>
            <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Quantity</label>
            <input className="craft-input" type="number" min="1" value={form.quantity} onChange={e => setForm(f => ({ ...f, quantity: e.target.value }))} />
          </div>
        </div>

        <div>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Preferred Color</label>
        ...
        <div>
          <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#5C3D2E", marginBottom: 6 }}>Design Description *</label>
        ...
        <button type="submit" className="btn-primary" disabled={loading || !user}
          style={{ justifyContent: "center", fontSize: 15, padding: "14px", opacity: (!user || loading) ? 0.6 : 1, background: "linear-gradient(135deg, #C97B63, #A85E48)" }}>
          <Shirt size={16} />
...
export default function CustomClothesPage() {
  return (
    <div style={{ background: "#F5E9DA" }}>
      {/* Hero */}
      <div style={{ background: "linear-gradient(135deg, #FDF6EE 0%, #F5E9DA 100%)", borderBottom: "1px solid #EDD9C540", padding: "60px 24px 56px" }}>
        <div style={{ maxWidth: 860, margin: "0 auto", textAlign: "center" }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(32px,5vw,52px)", fontWeight: 700, color: "#2C1A0E", marginBottom: 16 }}>
            Custom <span style={{ color: "#C97B63" }}>Clothes</span>
          </h1>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "56px 24px 0" }}>
        <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 700, color: "#2C1A0E", marginBottom: 8 }}>Ready-made Designs</h2>
        <p style={{ color: "#8B6F5E", marginBottom: 32 }}>Browse our existing collection or request a custom design below.</p>
      </div>
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "56px 24px 80px" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: 32, fontWeight: 700, color: "#2C1A0E", marginBottom: 8 }}>Want Something Custom?</h2>
          <p style={{ color: "#8B6F5E" }}>Tell us your design and we'll make it happen.</p>
        </div>
        <CustomOrderForm />
      </div>
    </div>
