...
function FAQItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderRadius: 16, background: "rgba(255,255,255,0.8)", border: "1px solid rgba(201,123,99,0.1)", marginBottom: 12, overflow: "hidden", transition: "all 0.3s ease", boxShadow: open ? "0 8px 24px rgba(201,123,99,0.1)" : "none" }}>
      <button
      ...
      >
        <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 16, fontWeight: 600, color: "#2C1A0E", lineHeight: 1.4 }}>{q}</span>
        <ChevronDown size={18} color="#C97B63" style={{ flexShrink: 0, transition: "transform 0.3s", transform: open ? "rotate(180deg)" : "rotate(0deg)" }} />
      </button>
      <div style={{ maxHeight: open ? 300 : 0, overflow: "hidden", transition: "max-height 0.4s cubic-bezier(0.16,1,0.3,1)" }}>
        <p style={{ padding: "0 24px 20px", fontSize: 15, color: "#6B4C3B", lineHeight: 1.7, margin: 0 }}>{a}</p>
      </div>
    </div>
  );
}
...
export default function FAQPage() {
  return (
    <div style={{ minHeight: "100vh", background: "#F5E9DA" }}>
      <div style={{ background: "linear-gradient(135deg, #FDF6EE 0%, #EDD9C5 100%)", borderBottom: "1px solid #bbf7d030", padding: "72px 24px 64px", textAlign: "center" }}>
        <div style={{ fontSize: 56, marginBottom: 16 }}>❓</div>
        <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(32px,5vw,52px)", fontWeight: 700, color: "#2C1A0E", marginBottom: 14 }}>Frequently Asked Questions</h1>
        <p style={{ fontSize: 17, color: "#8B6F5E", maxWidth: 500, margin: "0 auto" }}>Everything you need to know about Craft Verse. Can't find your answer? Contact us!</p>
      </div>

      <div style={{ maxWidth: 760, margin: "0 auto", padding: "64px 24px 80px" }}>
        {faqs.map((faq, i) => <FAQItem key={i} q={faq.q} a={faq.a} />)}

        <div style={{ textAlign: "center", marginTop: 56, padding: "40px", borderRadius: 24, background: "linear-gradient(135deg, #F5E9DA, #EDD9C5)", border: "1px solid rgba(201,123,99,0.1)" }}>
          <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, color: "#2C1A0E", marginBottom: 8 }}>Still have questions?</h3>
          <p style={{ color: "#8B6F5E", marginBottom: 20 }}>We're happy to help. Reach out via email or WhatsApp.</p>
